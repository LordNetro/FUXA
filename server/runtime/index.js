...
function init(_io, _api, _settings, _log, eventsMain) {
    ...
    recipeStorage.init(settings, logger, runtime).then(() => {
        ...
        logger.info('runtime init recipe services successful!', true);
    }).catch(err => {
        logger.error('runtime.failed-to-init recipe services: ' + err);
    });
        ...
        socket.on('recipe:cancel-execution', (message) => {
            try {
                ...
                if (message && message.recipeId) {
                    recipeService.cancelRecipe(message.recipeId);
                }
            } catch (err) {
